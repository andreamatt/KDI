...
# load classes
classes_txt = requests.get(
    'https://raw.githubusercontent.com/andreamatt/KDI/master/scripts/structure/classes.py').text
...
def rm_main(JSONString):
    ...
    for movie in cinemaRovereto['movies']:
        gen = GeneralEvent(movie['title'], movie['Price'], movie['description'],
                           movie['cinemaURL'], movie['length'], movie['language'], True)
        screen = ScreeningEvent(
            '', movie['originalName'], movie['genre'], movie['length'])
        work = CreativeWork(
            movie['title'], movie['director'], movie['createDate'], movie['URL'])

        for dateAndTime in movie['time']:
            date = Time(date['day'], date['day'],
                        date['startTime'], '')
